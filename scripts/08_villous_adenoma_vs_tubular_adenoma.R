...
output_dir <- here("output", "villous_vs_tubular")
...
res <- results(dds, contrast = c("tumor_type", "Villous_Adenoma", "Tubular_Adenoma"))
...
write.csv(
  deg_results,
  file.path(output_dir, "DEG_results_villous_vs_tubular.csv"),
  row.names = FALSE
)
...
volcano_plot <- ggplot(
  ...
  theme_minimal() +
  labs(
    title = "Volcano Plot: Villous Adenoma vs Tubular Adenoma",
    x = "Log2 Fold Change",
    y = "-Log10 Adjusted P-value"
  )

ggsave(
  filename = file.path(output_dir, "volcano_plot_villous_vs_tubular.pdf"),
  plot = volcano_plot,
  width = 8,
  height = 6
)
...
ma_plot <- ggplot(
  ...
  theme_minimal() +
  labs(
    title = "MA Plot: Villous Adenoma vs Tubular Adenoma",
    x = "Mean Expression (log scale)",
    y = "Log2 Fold Change"
  )

ggsave(
  filename = file.path(output_dir, "ma_plot_villous_vs_tubular.pdf"),
  plot = ma_plot,
  width = 8,
  height = 6
)
...
write.csv(
  top10_deg,
  file.path(output_dir, "top10_DEGs_villous_vs_tubular.csv"),
  row.names = FALSE
)