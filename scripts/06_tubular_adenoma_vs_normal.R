...
output_dir <- here("output", "tubular_vs_normal")
...
res <- results(dds, contrast = c("tumor_type", "Tubular_Adenoma", "Normal"))
...
write.csv(
  deg_results,
  file.path(output_dir, "DEG_results_tubular_vs_normal.csv"),
  row.names = FALSE
)
...
ma_plot <- ggplot(
  ...
  theme_minimal() +
  labs(
    title = "MA Plot: Tubular Adenoma vs Normal",
