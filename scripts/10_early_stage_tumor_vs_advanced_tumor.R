...
output_dir <- here("output", "early_stage_vs_advanced")
...
res <- results(dds, contrast = c("tumor_type", "Early_Stage_Tumor", "Advanced_Tumor"))
...
write.csv(
  deg_results,
  file.path(output_dir, "DEG_results_early_stage_vs_advanced.csv"),
  row.names = FALSE
)
...
volcano_plot <- ggplot(
  ...
  theme_minimal() +
  labs(
    title = "Volcano Plot: Early Stage Tumor vs Advanced Tumor",
    x = "Log2 Fold Change",
    y = "-Log10 Adjusted P-value"
  )

ggsave(
  filename = file.path(output_dir, "volcano_plot_early_stage_vs_advanced.pdf"),
  plot = volcano_plot,
  width = 8,
  height = 6
)
...
ma_plot <- ggplot(
  ...
  theme_minimal() +
  labs(
    title = "MA Plot: Early Stage Tumor vs Advanced Tumor",
    x = "Mean Expression (log scale)",
    y = "Log2 Fold Change"
  )

ggsave(
  filename = file.path(output_dir, "ma_plot_early_stage_vs_advanced.pdf"),
  plot = ma_plot,
  width = 8,
  height = 6
)
...
write.csv(
  top10_deg,
  file.path(output_dir, "top10_DEGs_early_stage_vs_advanced.csv"),
  row.names = FALSE
)